...
class Router
{
     ...
     * 1. Callable (anonymous function, invokable class, etc.)
     * 2. "Controller@method" string (assumes App\Controllers namespace)
     * 3. [ClassName::class, 'method'] array (fully qualified class name)
     *
     * @param mixed $handler
     * @param array $params
    ...
    protected function executeHandler(mixed $handler, array $params): void
    {
        ...
        // 2) String "Controller@method" (legacy style, adds namespace prefix)
        ...
        // 3) Array [ClassName::class, 'method'] (fully qualified, no namespace magic)
        if (is_array($handler) && count($handler) === 2) {
            ...
            if (is_string($controller) && class_exists($controller)) {
                ...
                throw new MachinjiriException("Invalid controller in route handler array: expected class string or object, got " . gettype($controller));
            }
    
            if (!method_exists($controllerInstance, $method)) {
                throw new MachinjiriException("Method '$method' not found in controller '" . get_class($controllerInstance) . "'");
            }
