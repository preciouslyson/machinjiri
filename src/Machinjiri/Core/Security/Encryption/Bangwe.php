...
class Bangwe
{
    ...
    private function getEncryptionKey(): string
    {
        ...
        // Heuristic: if the key looks like Base64 (no whitespace, only base64 chars, length multiple of 4)
        if (preg_match('#^[a-zA-Z0-9+/]*={0,2}$#', $key) && strlen($key) % 4 === 0) {
        ...
        if (strlen($key) < 32) {
            $key = str_pad($key, 32, "\0");   // Note: use null bytes, not '0'
    ...
    */
    ...
    private function getEncryptionKey(): string
    {
        ...
        // If decoding fails or produces empty, maybe it was already raw
        if ($key === false || $key === '') {
            $key = $this->encryptionKey;
        }
        
        // Pad or truncate? Better to throw if not exactly 32 bytes
        if (strlen($key) !== 32) {
