...
class Router
{
    ...
    // AJAX-only route
    public function ajax(string $pattern, mixed $handler, ?string $name = null, array $options = []): self
    {
        $options['ajax_only'] = true;
        return $this->map(['GET', 'POST', 'PUT', 'DELETE', 'PATCH'], $pattern, $handler, $name, $options);
    }

    // Non-AJAX route (traditional form submissions)
    public function traditional(string $pattern, mixed $handler, ?string $name = null, array $options = []): self
    {
